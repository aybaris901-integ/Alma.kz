...
import { parseSlotTime } from "../lib/parseSlotTime";
...
export default function OrderStatusPage() {
  ...
  useEffect(() => {
    ...
    // Shared by the initial load and every realtime tick below, so a
    // status change made on the restaurant's dashboard shows up here
    // without the guest needing to manually reload the page.
    function loadOrder(showSpinner: boolean) {
      if (showSpinner) {
        setLoading(true);
        setError(null);
      }
    ...
    // Realtime subscribes to the base `orders` table (views can't be
    // subscribed to), filtered to just this order. On any change —
    // status advanced on the Partner dashboard, picked up via the QR
    // screen, etc. — refetch through `orders_with_items` so `items`
    // stays populated, rather than trying to patch the payload by hand.
  ...
  const pickupTime = slot
    ? parseSlotTime(slot.slotTime).toLocaleTimeString("ru-RU", {
        hour: "2-digit",
        minute: "2-digit",
      })
    : "—";
